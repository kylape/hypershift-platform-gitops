...
│   ├── stackrox-operator/
│   │   ├── namespace.yaml        # stackrox namespace
│   │   ├── operator.yaml         # StackRox operator (CRDs, RBAC, Deployment)
│   │   └── central.yaml          # Central CR with VM scanning enabled
...
* **StackRox Operator**: RHACS operator deployed directly (not via OLM)
  * Operator manages Central, Scanner V4, and secured cluster components
  * Central CR configured with VM scanning support (`ROX_VIRTUAL_MACHINES=true`)
  * Certificates and secrets are auto-generated by the operator
